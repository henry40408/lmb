...
mod tests {
    ...
    fn test_read_all(text: &'static str) {
    ...
    fn test_read_count(bytes: &'static [u8], expected: Value) {
        let source = include_str!("fixtures/read-count.lua");
        let input = Cursor::new(bytes);
        let runner = Runner::builder(&source, input).build().unwrap();
        assert_eq!(expected, runner.invoke().call().unwrap().result.unwrap());
    }
    ...
    fn test_read_line(text: &'static str, expected: &'static str) {
    ...
    fn test_read_number(text: &'static str, expected: Value) {
